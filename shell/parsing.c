...
expand_environ_var(char* arg) {
    if (arg == NULL)
        perror("expand_environ_var: arg sent is null");
	// Your code here
    char args[BUFLEN];
	if (arg[0] == '$') {
        arg = shift_left(arg, 1);
        //args = (char *)calloc(ARGSIZE, sizeof(char));
        if (strcmp(arg, "?") == 0) {
            sprintf(args, "%d", status);
            arg = strcpy(arg, args);
        }
        else {
            char *argss = getenv(arg);
            arg = strcpy(arg, argss);
        }
        return arg;
	}
    return arg;
